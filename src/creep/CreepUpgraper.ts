...
export default class CreepUpgrader extends BaseCreep {
  ...
  public source() {
    ...
    let source: any
    let controllerLink = this.myRoom.controllerLink
    if (controllerLink && controllerLink.store.getUsedCapacity(RESOURCE_ENERGY) > 0) {
      if (this.withdraw(controllerLink, RESOURCE_ENERGY) == ERR_NOT_IN_RANGE) {
        this.goTo(controllerLink.pos)
